...
module.exports.handleEvent = async ({ api, event, args }) => {
  // This bot doesn't handle events, so we leave this function empty
};
    ...
    if (os.platform() === 'linux') {
      exec('acpi -b', (error, stdout, stderr) => {
        if (!error) {
          ...
          const batteryPercentageMatch = batteryInfo.match(/\d+%/);
          if (batteryPercentageMatch !== null) {
            batteryPercentage = batteryPercentageMatch[0];
          }
        } else {
